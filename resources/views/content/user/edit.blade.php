...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
    @endcomponent
    <form action="{{route('user.update', ['id'=> $admin->id])}}" method="POST" enctype="multipart/form-data" class="frm_form_add">
    @csrf
    <div class="row">
        <div class="col-12">
            <div class="card">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="mb-3">
                                ...
                                <input id="name" name="name" type="text" class="form-control" value="{{$admin->name}}" placeholder="">
                            </div>
                            <div class="mb-3">
                                <label for="metatitle">email</label>
                                <input id="metatitle" name="email" type="text" class="form-control" value="{{$admin->email}}" disabled
                                       placeholder="">
                            </div>
                            <div class="mb-3">
                                <label for="metakeywords">Password</label>
                                <input id="metakeywords" name="password" type="password" class="form-control" value="" disabled
                                       placeholder="">
                            </div>
                            <div class="mb-3">
                                <label for="metakeywords">Vai trò</label>
                                {!! Form::select('role_id', ['' => 'Chọn vai trò'] + $listRole, old('role_id', $admin->role_id), ['class' => 'form-control select2']) !!}
                            </div>
                            <div class="mb-3">
                                <label for="metakeywords">Trạng thái</label>
                                {!! Form::select('status', ['' => 'Chọn trạng thái'] + \App\Models\Admin::$listStatus, old('status', $admin->status), ['class' => 'form-control select2']) !!}
                            </div>
                        ...
                        <div class="mb-3 col-sm-6">
                            ...
                            <div class="row justify-content-center">
                                <img id="img_preview" src="{{ $admin->avatar ?? asset('build/images/default-preview-img.jpg') }}" ><br>
